...
// Med-Spa CTA primary btn animation
document.querySelectorAll(".medspa-bubble-btn").forEach((button) => {
  const circle = button.querySelector(".medspa-bubble-btn__circle");
...
// Medspa marqueee animation slider
document.addEventListener("DOMContentLoaded", function () {
    const saloneandspasMarquee = document.querySelector("#medspa-marquee-slider");

    if (saloneandspasMarquee) {
        new Splide(saloneandspasMarquee, {
            type: "loop",
            perPage: 5,
            perMove: 1,
            gap: "2rem",
...
// Medspa Banner stagger animation
document.addEventListener("DOMContentLoaded", function () {
  const heroTl = gsap.timeline({
    defaults: { ease: "power3.out", duration: 1 }
  });

  gsap.set(".medspa-hero-title", { display: "inline-block" });

  heroTl
    .from(".medspa-hero-title", {
      yPercent: 60,
      opacity: 0,
      filter: "blur(10px)",
      duration: 1.1,
    ...
    .from(".medspa-hero-image", {
      scale: 0.92,
      opacity: 0,
      duration: 1.2
    }, "-=0.9")
    .from(".medspa-hero-checklist", {
      y: 30,
      opacity: 0
    }, "-=0.9")
    .from(".medspa-hero-checklist-item", {
      y: 15,
      opacity: 0,
      stagger: 0.12,
      duration: 0.7
    }, "-=0.5")
    .from(".medspa-hero-description", {
      y: 30,
      opacity: 0
    }, "-=0.9")
    .from(".medspa-hero-iconbox-item", {
      y: 20,
      opacity: 0,
      stagger: 0.15
    }, "-=0.5");
